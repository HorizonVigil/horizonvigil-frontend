...
import { RealRepositoryAssetDetail } from './sourceInventory/RealRepositoryAssetDetail';
...
export function SourceAssetDetail() {
  ...
  // unused on the real-data branches below -- asset is always null there,
  // so this resolves to [] immediately, no wasted work.
  const findings = useMemo(() => (asset ? generateAggregatedFindings(asset) : []), [asset]);

  // Same real/mock split as SourceInventoryCategory.tsx -- Clouds and
  // Repositories are real now. Placed after every hook above so this
  // component calls the same hooks in the same order on every render
  // regardless of which category the route is currently on (React Router
  // re-renders this same component instance in place when only
  // :category/:assetId change).
  if (category === 'cloud') return assetId ? <RealCloudAssetDetail assetId={assetId} /> : null;
  if (category === 'repository') return assetId ? <RealRepositoryAssetDetail assetId={assetId} /> : null;
