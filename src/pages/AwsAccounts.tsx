...
export function AwsAccounts() {
  ...
  const columns: Column<CloudConnection>[] = [
    ...
    { key: 'status', header: 'Status', render: c => <Badge>{c.status}</Badge>, sortValue: c => c.status },
    ...
    { key: 'lastSync', header: 'Last Sync', render: c => c.lastSyncAt ? new Date(c.lastSyncAt).toLocaleString() : 'Never', sortValue: c => c.lastSyncAt ?? '' },
    {
      key: 'actions', header: 'Actions', render: c => (
        <div className="flex gap-2 text-xs">
          <button onClick={e => { e.stopPropagation(); void handleSyncNow(c.id); }} disabled={busyId === c.id} className="text-brand-600 dark:text-brand-400 hover:underline disabled:opacity-50">{busyId === c.id ? 'Syncing…' : 'Sync Now'}</button>
