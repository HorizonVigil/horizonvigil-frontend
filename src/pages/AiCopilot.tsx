...
import { pendingSignals, safeAdvisorHref, type AdvisorAnswer, type AdvisorDecision, type AdvisorMode, type AdvisorSignal, type AdvisorWorkspace, type DecisionOutcome, type DecisionStatus } from '../lib/advisor';
import { AdvisorConversations } from './AdvisorConversations';

type View = 'overview' | 'queue' | 'advisor' | 'decisions' | 'outcomes' | 'evidence' | 'governance';
...
function Governance({ workspace }: { workspace: AdvisorWorkspace }) {
  const governance = workspace.governance;
  const controls = [
    ['Provider boundary', governance.provider, 'Only AWS connections and evidence enter this production workspace.'],
    ['Human approval', governance.humanApprovalRequired ? 'Required' : 'Disabled', 'AI advice cannot authorize or execute a cloud change.'],
    ['Cloud mutation', governance.cloudMutationEnabled ? 'Enabled' : 'Disabled', 'The Advisor records decisions and remains read-only.'],
    ['Inference audit', governance.inferenceAuditEnabled ? 'Enabled' : 'Disabled', 'Prompt, model, evidence IDs, latency, fallback state and output hash are recorded.'],
    ['Decision history', governance.appendOnlyDecisions ? 'Append-only' : 'Mutable', 'Decisions and outcomes remain attributable and cannot be overwritten.'],
    ['Outcome tracking', governance.outcomeTrackingEnabled ? 'Enabled' : 'Disabled', 'Reviewers can evaluate whether recorded decisions were effective.'],
  ];
  return <div className="space-y-4">
    <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900 dark:bg-emerald-950/30"><div className="flex items-center gap-2"><Icon name="shield-check" size={18}/><h2 className="font-semibold text-emerald-950 dark:text-emerald-100">AWS production governance is active</h2></div><p className="mt-2 text-sm text-emerald-900/80 dark:text-emerald-200">Prompt <strong>{governance.promptVersion}</strong> · Model <strong>{governance.modelId}</strong> · Evaluation suite <strong>{governance.evaluationSuite}</strong></p></div>
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{controls.map(([label, value, detail]) => <div key={label} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><div className="text-xs font-semibold uppercase tracking-wide text-slate-400">{label}</div><div className="mt-2 text-lg font-semibold text-slate-950 dark:text-white">{value}</div><p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">{detail}</p></div>)}</div>
  </div>;
}

function Outcomes({ workspace, onReload }: { workspace: AdvisorWorkspace; onReload: () => Promise<void> }) {
  const { toast } = useToast();
  const [target, setTarget] = useState<string | null>(null);
  const [outcome, setOutcome] = useState<Exclude<DecisionOutcome, 'not_evaluated'>>('effective');
  const [notes, setNotes] = useState('');
  const [saving, setSaving] = useState(false);

  async function save(decision: AdvisorDecision) {
    if (!notes.trim()) return;
    setSaving(true);
    try {
      await api.recordAdvisorOutcome(decision.id, { outcome, notes: notes.trim() });
      await onReload();
      setTarget(null); setNotes('');
      toast('Outcome recorded with the current AWS evidence snapshot.', 'success');
    } catch (cause) { toast(friendlyErrorMessage(cause, 'The outcome could not be recorded.'), 'error'); }
    finally { setSaving(false); }
  }

  if (workspace.decisions.length === 0) return <EmptyState icon="chart-line" title="No decisions to evaluate" description="Record a governed decision first. Outcomes compare the later AWS observation with that decision." />;
  return <div className="space-y-3">{workspace.decisions.map(decision => <div key={decision.id} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><div className="flex flex-wrap items-center gap-2"><Badge>{decision.status}</Badge><Badge>{decision.outcome.replace(/_/g, ' ')}</Badge><span className="text-xs text-slate-400">Owner: {decision.owner}</span></div><h3 className="mt-2 font-semibold text-slate-950 dark:text-white">{decision.signal_title}</h3>{decision.outcome_notes && <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{decision.outcome_notes}</p>}{decision.outcome_at && <p className="mt-1 text-xs text-slate-400">Evaluated {formatDate(decision.outcome_at)}</p>}
    {target === decision.id ? <div className="mt-4 border-t border-slate-100 pt-4 dark:border-slate-800"><div className="grid grid-cols-3 gap-2">{(['effective','ineffective','inconclusive'] as const).map(value => <button key={value} type="button" onClick={() => setOutcome(value)} className={`rounded-lg border px-2 py-2 text-xs font-semibold capitalize ${outcome === value ? 'border-brand-600 bg-brand-50 text-brand-700 dark:bg-brand-950/30 dark:text-brand-300' : 'border-slate-300 text-slate-600 dark:border-slate-700 dark:text-slate-300'}`}>{value}</button>)}</div><label className="mt-3 block text-xs font-medium text-slate-600 dark:text-slate-300">Outcome evidence and reasoning<textarea value={notes} onChange={event => setNotes(event.target.value)} maxLength={2000} rows={3} className="mt-1 w-full rounded-lg border border-slate-300 bg-white p-2 text-sm text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></label><div className="mt-3 flex gap-2"><button type="button" onClick={() => void save(decision)} disabled={saving || !notes.trim()} className="rounded-lg bg-brand-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? 'Recording…' : 'Record outcome'}</button><button type="button" onClick={() => setTarget(null)} className="rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-slate-700">Cancel</button></div></div> : <button type="button" onClick={() => { setTarget(decision.id); setNotes(''); }} disabled={!workspace.canDecide} className="mt-3 text-sm font-semibold text-brand-600 disabled:opacity-50 dark:text-brand-400">{decision.outcome === 'not_evaluated' ? 'Evaluate outcome' : 'Add a newer evaluation'}</button>}
  </div>)}</div>;
}
...
export function AiCopilot() {
  ...
  function openSignal(signal: AdvisorSignal) { setSelected(signal); setAnswer(null); setRationale(''); setOwner(''); setReviewAt(''); setDecisionStatus('approved'); }
  ...
  async function saveDecision() {
    if (!selected || !workspace || !rationale.trim() || !owner.trim() || (decisionStatus === 'deferred' && !reviewAt)) return;
    setSaving(true);
    try {
      const record = await api.recordAdvisorDecision({ signalId: selected.id, status: decisionStatus, rationale: rationale.trim(), owner: owner.trim(), reviewAt: decisionStatus === 'deferred' ? new Date(reviewAt).toISOString() : null, advisorRunId: answer?.runId ?? null });
  ...
  if (!workspace) return <EmptyState icon="ai" title="Live Intelligence is temporarily unavailable" description={error ?? 'The production Advisor service did not return its AWS evidence workspace.'} action={{ label: 'Retry live AWS evidence', onClick: () => void load() }} />;
  ...
  return <div key={scopeKey} className="space-y-5 pb-10">
    <section className="relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 px-5 py-6 text-white shadow-lg sm:px-7">
      ...
      <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><div className="flex items-center gap-2 text-xs font-semibold tracking-[0.18em] text-brand-300"><Icon name="sparkles" size={16}/> AWS INTELLIGENCE · PRODUCTION GOVERNED</div><h1 className="mt-3 max-w-3xl text-3xl font-semibold tracking-tight sm:text-4xl">Your AWS cloud. Every decision, explained.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">A read-only decision layer over AWS cost, security and operations. Every answer is evidence-scoped and auditable; every decision remains human-controlled.</p></div>
    ...
    {view === 'advisor' ? <AdvisorConversations /> : view === 'evidence' ? <Coverage workspace={workspace} /> : view === 'governance' ? <Governance workspace={workspace} /> : view === 'decisions' ? <div className="space-y-3">{workspace.decisions.length === 0 ? <EmptyState icon="scroll-text" title="No decisions recorded" description="Review a signal and record the team's rationale. The advisor never applies cloud changes." /> : workspace.decisions.map(item => <div key={item.id} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><div className="flex flex-wrap items-center gap-2"><Badge>{item.status}</Badge><Badge>{item.outcome.replace(/_/g, ' ')}</Badge><span className="text-xs text-slate-400">{formatDate(item.created_at)}</span></div><h3 className="mt-2 font-semibold text-slate-950 dark:text-white">{item.signal_title}</h3><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{item.rationale}</p><p className="mt-2 text-xs text-slate-400">Accountable owner: {item.owner}</p>{item.review_at && <p className="mt-1 text-xs text-slate-400">Review on {formatDate(item.review_at)}</p>}{item.advisor_run_id && <p className="mt-1 font-mono text-[10px] text-slate-400">Advisor run {item.advisor_run_id}</p>}</div>)}</div> : view === 'outcomes' ? <Outcomes workspace={workspace} onReload={load} /> : <>
    ...
    <Drawer open={selected !== null} onClose={() => setSelected(null)} title={selected?.title ?? 'Signal review'} wide>{selected && <div className="space-y-5">
      ...
      <div><div className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Ask the advisor</div><div className="grid grid-cols-3 gap-2">{(['explain','verify','advise'] as AdvisorMode[]).map(mode => <button key={mode} type="button" disabled={answerLoading} onClick={() => void ask(mode)} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold capitalize text-slate-700 hover:border-brand-500 hover:text-brand-700 disabled:opacity-50 dark:border-slate-700 dark:text-slate-200">{mode}</button>)}</div>{answerLoading && <p className="mt-3 text-sm text-slate-500">Reviewing the retrieved evidence…</p>}{answer && <div className="mt-3 rounded-xl border border-brand-200 bg-brand-50 p-4 text-sm leading-6 text-slate-800 dark:border-brand-900 dark:bg-brand-950/30 dark:text-slate-200"><div className="mb-1 flex items-center justify-between"><strong className="capitalize">{answer.mode}</strong><span className="text-xs text-slate-400">{answer.engine}</span></div>{answer.answer}<div className="mt-3 border-t border-brand-200 pt-2 font-mono text-[10px] text-slate-500 dark:border-brand-900">Run {answer.runId} · {answer.promptVersion} · {answer.modelId}</div>{answer.limitations.length > 0 && <ul className="mt-3 list-disc pl-5 text-xs text-slate-500">{answer.limitations.map(item => <li key={item}>{item}</li>)}</ul>}</div>}</div>
      ...
      {workspace.decisionsAvailable && <div className="border-t border-slate-200 pt-5 dark:border-slate-800"><h3 className="font-semibold text-slate-950 dark:text-white">Record the human decision</h3><p className="mt-1 text-xs text-slate-500">This creates an append-only audit record. It does not change any AWS resource.</p><div className="mt-3 grid grid-cols-3 gap-2">{(['approved','dismissed','deferred'] as DecisionStatus[]).map(status => <button key={status} type="button" onClick={() => setDecisionStatus(status)} className={`rounded-lg border px-2 py-2 text-xs font-semibold capitalize ${decisionStatus === status ? 'border-brand-600 bg-brand-50 text-brand-700 dark:bg-brand-950/30 dark:text-brand-300' : 'border-slate-300 text-slate-600 dark:border-slate-700 dark:text-slate-300'}`}>{status}</button>)}</div><label className="mt-3 block text-xs font-medium text-slate-600 dark:text-slate-300">Accountable owner<input value={owner} onChange={event => setOwner(event.target.value)} maxLength={200} className="mt-1 w-full rounded-lg border border-slate-300 bg-white p-2 text-sm text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-white" placeholder="Team or person responsible for follow-through" /></label><label className="mt-3 block text-xs font-medium text-slate-600 dark:text-slate-300">Rationale<textarea value={rationale} onChange={event => setRationale(event.target.value)} maxLength={2000} rows={3} className="mt-1 w-full rounded-lg border border-slate-300 bg-white p-2 text-sm text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-white" placeholder="What did the reviewer decide and why?" /></label>{decisionStatus === 'deferred' && <label className="mt-3 block text-xs font-medium text-slate-600 dark:text-slate-300">Review date<input type="datetime-local" value={reviewAt} onChange={event => setReviewAt(event.target.value)} className="mt-1 block w-full rounded-lg border border-slate-300 bg-white p-2 text-sm dark:border-slate-700 dark:bg-slate-900" /></label>}<button type="button" onClick={() => void saveDecision()} disabled={saving || !owner.trim() || !rationale.trim() || (decisionStatus === 'deferred' && !reviewAt) || !workspace.canDecide} className="mt-3 w-full rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-50">{saving ? 'Recording…' : workspace.canDecide ? 'Record governed decision' : 'Viewer access — decisions disabled'}</button></div>}
    </div>}</Drawer>
  </div>;
}
